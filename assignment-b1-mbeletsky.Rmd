...
I've included "datateachr" so that we have access to the "vancouver_trees" dataset I'll be using as an example. 
...
```
Here we have a nice table showing how many distinct tree species there are in each Vancouver neighbourhood. We find out that Hastings-Sunrise, Kitsilano, and Renfrew-Collingwood have the most unique tree species while Fairview and Downtown have the least. 
...
```
Now we have a nice table showing how many distinct tree species there are in Vancouver in each genus. The genera with the most unique species found in Vancouver are Acer, Prunus, and Magnolia.
...
```
Now that we've grouped by genus and summarized unique cultivars, we see that Acer, Prunus, and Magnolia have the most unique cultivars within them. 
...
We can use the test function **expect_output()** to test that our function is outputting the "table" tibble, as we want it to. 
